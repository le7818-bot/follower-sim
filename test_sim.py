...
# [수정] 슬라이더를 number_input으로 변경
daily_base = st.sidebar.number_input("일일 확정 획득량", min_value=10000, max_value=100000, value=30000, step=5000)
pvp_min = st.sidebar.number_input("추가 획득 (최소)", min_value=0, max_value=50000, value=5000, step=1000)
pvp_max = st.sidebar.number_input("추가 획득 (최대)", min_value=5000, max_value=200000, value=50000, step=5000)
...
for name, val in zip(tier_names, threshold_vals):
    # [수정] ZeroDivisionError 방지
    days_hard = int(val / daily_hard) if daily_hard > 0 else 0
    days_avg = int(val / daily_avg) if daily_avg > 0 else 0
    days_light = int(val / daily_light) if daily_light > 0 else 0
